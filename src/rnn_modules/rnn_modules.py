...
def _get_rnn_last_output(seq_len, output):
    ...
    if CUDA:
        tmp_indices = Variable(torch.cuda.LongTensor(range(seq_len.size(0))))
    else:
        tmp_indices = Variable(torch.LongTensor(range(seq_len.size(0))))
...
class RNNClassifier(nn.Module):
    def __init__(self, features_dim, hidden_dim, config):
        super().__init__()
        self.config = config
        self.hidden_dim = hidden_dim

        # load embeddings
        self.rnn = nn.RNN(features_dim, hidden_dim, num_layers=1, nonlinearity='tanh',
                          dropout=0.2)
    ...
    def forward(self, seq_tensor, seq_lengths, hidden=None):
        ...
        print(outputs[:3].size(), lengths[:3])
        # print(outputs[-1].size())
        # x = self.linear(_get_rnn_last_output(Variable(seq_lengths), outputs))
...
def generate_dataloader(dataset_tensor, lengths_tensor,  labels, batch_size, ratio_train_val_set =
    0.9):
    if CUDA:
        labels_tensor = torch.cuda.FloatTensor(labels)
        indeces = torch.randperm(len(labels)).cuda()
...
def sort_sequences(seq_tensor, seq_lengths, labels):
    seq_lengths, perm_idx = seq_lengths.sort(0, descending=True)
    seq_tensor = seq_tensor[perm_idx]
    # print(seq_tensor[:10], seq_lengths[:10])
...
def train(model, embed, dataloader_train, train_lengths, config, dataloader_val=None,
          val_lengths=None):
    iter_counter = 0
    ...
    for epoch in range(config.epochs_num):
        ...
        for index, sample_batched in enumerate(dataloader_train):
            ...
            seq_tensor, seq_lengths, labels = sort_sequences(seq_tensor, seq_lengths, labels)
            
            if CUDA:
                labels = Variable(labels.type(torch.cuda.LongTensor), requires_grad=False)
                seq_var = Variable(seq_tensor.type(torch.cuda.LongTensor), requires_grad=False)
            ...
            seq_var = embed(seq_var)
            
            output, hidden = model(seq_var, seq_lengths, hidden)
        ...
        model.eval()
        train_accuracy, train_loss = test(model, embed, dataloader_train, train_lengths, config)
        ...
        if dataloader_val:
            val_accuracy, val_loss = test(model, embed, dataloader_val, val_lengths, config)
...
def test(model, embed, dataloader, lengths, config):
    loss = 0
    accuracy = 0
    ...
    for index, sample_batched in enumerate(dataloader):
        ...
        loss += model.loss(output, labels).data[0]
    return accuracy/num_batches, loss/num_batches
...
def run():
    ...
    if CUDA:
        model = RNNClassifier(embeddings.shape[1], 20, config).cuda()
    else:
        model = RNNClassifier(embeddings.shape[1], 20, config)
    print("model loaded. Start training data...")

    embed = nn.Embedding(*embeddings.shape)
    if CUDA:
        embed.weight = nn.Parameter(torch.cuda.FloatTensor(embeddings))
    else:
        embed.weight = nn.Parameter(torch.FloatTensor(embeddings))

    embed.weight.requires_grad = False

    train_loss_history, train_accuracy_history, val_loss_history, val_accuracy_history = train(
        model, embed, dataloader_train, lengths_train, config,
